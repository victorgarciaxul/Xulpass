  ...
  var require_index = __commonJS({
    "src/popup/index.ts"() {
      ...
      $("btn-login").addEventListener("click", async () => {
        ...
        $("btn-login").textContent = "Iniciar sesi\xF3n";
        if (error || !data.user) {
          errEl.textContent = "Email o contrase\xF1a incorrectos";
          return;
        }
      ...
      $("btn-add-entry").addEventListener("click", async () => {
        try {
          ...
          if (url.startsWith("http")) {
            ...
            $("add-url").value = url;
            const domain = new URL(url).hostname.replace("www.", "");
            $("add-title").value = domain.split(".")[0].charAt(0).toUpperCase() + domain.split(".")[0].slice(1);
          }
        } catch {
        }
      ...
      $("btn-gen").addEventListener("click", () => {
        ...
        const input = $("add-password");
        input.value = pwd;
        input.type = "text";
        setTimeout(() => input.type = "password", 2e3);
        showToast("Contrase\xF1a generada");
      });
      $("btn-save").addEventListener("click", async () => {
        ...
        const favorite = $("add-favorite").checked;
        $("save-error").textContent = "";
        if (!title || !username || !password) {
          $("save-error").textContent = "T\xEDtulo, usuario y contrase\xF1a son obligatorios";
          return;
        }
        const btn = $("btn-save");
        ...
        btn.textContent = "Guardar contrase\xF1a";
        btn.disabled = false;
        if (error) {
          $("save-error").textContent = "Error al guardar. Int\xE9ntalo de nuevo.";
          return;
        }
        ;
        ...
        $("add-favorite").checked = false;
        await loadEntries();
        showScreen("vault");
        showToast("\xA1Contrase\xF1a guardada!");
      });
      function renderEntries() {
        ...
        const filtered = allEntries.filter((e) => {
          ...
          const matchTab = currentTab === "all" ? true : currentTab === "favorites" ? e.favorite : e.category === currentTab;
    ...
    <div class="entry-card" data-id="${e.id}">
      ...
      <div class="entry-info">
        ...
        <div class="entry-pwd" id="pwd-${e.id}">
          ${revealedId === e.id ? `<span class="pwd-text">${esc(e.encrypted_password)}</span>` : '<span class="pwd-dots">\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022</span>'}
        </div>
      </div>
      <div class="entry-actions">
        <button class="btn-icon btn-reveal" data-id="${e.id}" title="${revealedId === e.id ? "Ocultar" : "Ver"}">
          ${revealedId === e.id ? "\u{1F648}" : "\u{1F441}\uFE0F"}
        </button>
        <button class="btn-icon btn-copy-user" data-val="${esc(e.username)}" title="Copiar usuario">\u{1F464}</button>
        <button class="btn-icon btn-copy-pwd" data-id="${e.id}" data-val="${esc(e.encrypted_password)}" title="Copiar contrase\xF1a">
          ${copiedId === e.id ? "\u2705" : "\u{1F511}"}
        </button>
